  ...
  >
    <template v-slot:top-left v-if="searchable">
      <VForm class="flex items-center q-gutter-md" @submit="search">
        <InputField class="w-[300px]" v-model="searchValue" name="search" label="Search by name here..." customIconClass="search__custom-icon" :outlined="false">
          <template #append>
            <Icon name="material-symbols:search" class="cursor-pointer" @click="search"> </Icon>
          </template>
        </InputField>
      </VForm>
    </template>
    ...
    <template v-for="(slotName, columnIndex) in dynamicSlots" v-slot:[`body-cell-${slotName}`]="props">
      <!-- Render custom content for this slot -->
      <slot :name="slotName" :columnIndex="columnIndex" v-bind="props" />
    </template>
    <slot></slot>
    <template #loading>
    ...
    <template #bottom v-if="dynamoDBPagination">
      <div class="w-full flex justify-center gap-4">
        <Icon name="material-symbols:chevron-left-rounded" class="cursor-pointer" width="24" height="24" @click="emit('prevPage')" />
        <Icon name="material-symbols:chevron-right-rounded" class="cursor-pointer" width="24" height="24" @click="emit('nextPage')" />
      </div>
    </template>
  </q-table>
</template>
...
const emit = defineEmits(["search", "sort", "filter", "paginate", "prevPage", "nextPage"]);
